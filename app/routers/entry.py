...
async def route_entry_post(entry: Entry, response: Response):
  ...
  if entry.encrypted:
    ...
    if len(recipients) == 0:
      ...
      get_database().add_to_db(f'fingerprint:{entry.fingerprint}:message:{hash_key(entry.key)}', entry.message)
    ...
    if not encrypt_result.ok:
      ...
      get_database().add_to_db(f'fingerprint:{entry.fingerprint}:message:{hash_key(entry.key)}', encrypt_result.data)
...
async def route_entry_get(fingerprint: str, key: str, response: Response):
  result = get_database().get_from_db(f'fingerprint:{fingerprint}:message:{hash_key(key)}')
  if result == None:
...
async def route_entry_delete(fingerprint: str, key: str, response: Response):
  result = get_database().delete_from_db(f'fingerprint:{fingerprint}:message:{hash_key(key)}')
  if result == None:
    response.status_code = status.HTTP_404_NOT_FOUND
  return {"result": result}
