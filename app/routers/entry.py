...
async def route_entry_post(entry: Entry, response: Response):
  if entry.encrypted:
    ...
    if len(recipients) == 0:
      ...
      get_database().add_to_db(f'{entry.fingerprint}:message:{entry.key}', entry.message)
    ...
    if not encrypt_result.ok:
      ...
      get_database().add_to_db(f'{entry.fingerprint}:message:{entry.key}', encrypt_result.data)
...
async def route_entry_get(fingerprint: str, key: str, response: Response):
  result = get_database().get_from_db(f'{fingerprint}:message:{key}')
  if result == None:
